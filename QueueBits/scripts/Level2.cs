...
namespace QueueBits
{
	public class Level2 : MonoBehaviour
	{
		...
		void CreateField()
		{
			...
			tutorial_text.GetComponent<TextMesh>().text = "Welcome to Level 2! \nThe rules of this game are the same as \nin regular Connect 4 — but with a twist! \nInstead of having purely red or yellow pieces, \nyou will now drop pieces that have a \nprobability of being red or yellow. Once \n4 red pieces or 4 yellow pieces \nare connected in a row, the game is over. Have fun!";
...
(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f), btnNextLevel.transform.position.z);
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
				...
				btnPlayAgain.GetComponent<TextMesh>().fontSize = 32;

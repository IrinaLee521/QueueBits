...
namespace QueueBits
{
	public class Level5 : MonoBehaviour
	{
		...
		void CreateField()
		{
			...
			tutorial_text = Instantiate(pieceCounterText, new Vector3(-5.0f, -0.5f, -1), Quaternion.identity) as GameObject;
			tutorial_text.GetComponent<TextMesh>().color = Color.black;
			tutorial_text.GetComponent<TextMesh>().text = "Welcome to Level 5! \nThe rules are getting zanier but it’s still Connect 4. \nThis time, when you drop a piece it won’t reveal itself to \nbe red or yellow, but stay in superposition. You need to keep \ndropping pieces until the whole board fills up. Once the whole board \nis filled, the pieces will break their superposition and reveal \nthemselves to be red or yellow in the order that the pieces were \ndropped. Once 4 red pieces or 4 yellow pieces are connected in a \nrow, the game is over. Have fun!";
